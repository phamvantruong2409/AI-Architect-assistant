...
export function EntryGrid({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/projects/${projectId}`)
      .then((res) => res.json())
      .then((data) => setFolderPath(typeof data?.folderPath === "string" ? data.folderPath : null))
      .catch(() => {});
  }, [projectId]);
  ...
  const loadEntries = useCallback((relPath: string) => {
    setLoading(true);
    setError(null);
    fetch(`/api/projects/${projectId}/entries?path=${encodeURIComponent(relPath)}`)
      .then((res) => res.json())
      .then((data: ProjectEntry[]) => setEntries(data))
  ...
  async function handleOpenEntry(entry: ProjectEntry) {
    if (entry.type === "folder") {
      setCurrentPath(entry.path);
      return;
    }
    await fetch(`/api/projects/${projectId}/entries/open`, {
      method: "POST",
  ...
  function handleContextMenu(event: React.MouseEvent, entry: ProjectEntry, index: number) {
  ...
  function handleEntryClick(event: React.MouseEvent, entry: ProjectEntry, index: number) {
  ...
  async function handleDeleteEntries(paths: string[]) {
    ...
    const res = await fetch(`/api/projects/${projectId}/entries/delete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  function startRename(entry: ProjectEntry) {
    setRenamingPath(entry.path);
    setRenameValue(entry.name);
    setContextMenu(null);
  }

  async function submitRename(entry: ProjectEntry) {
    ...
    const res = await fetch(`/api/projects/${projectId}/entries/rename`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  function handleRenameKeyDown(event: KeyboardEvent<HTMLInputElement>, entry: ProjectEntry) {
    if (event.key === "Enter") {
      event.preventDefault();
      submitRename(entry);
  ...
  async function uploadFiles(files: DroppedFile[]) {
    ...
    try {
      ...
      const res = await fetch(`/api/projects/${projectId}/entries/upload`, {
        method: "POST",
        body: formData,
      });
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1 text-sm">
        <button
          ...
          className={currentPath === "" ? "font-medium text-foreground" : "font-medium text-accent hover:underline"}
        >
          {folderPath ?? "..."}
        </button>
      ...
      >
        ...
        {!loading && entries.length === 0 && (
          <p className="text-sm text-foreground-soft">Thư mục này đang trống.</p>
        )}
              ...
              >
                {isImage ? (
                  <div
                    ...
                    style={{
                      backgroundImage: `url(/api/local-file?id=${projectId}&path=${encodeURIComponent(entry.path)})`,
                    }}
                  />
                ) : (
