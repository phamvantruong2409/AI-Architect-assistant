import { analyzeRender } from "@/lib/review-gemini";
import { geminiErrorCode, geminiErrorMessage } from "@/lib/gemini-error";
import { MAX_IMAGE_BYTES, type ReviewRequest } from "@/lib/review-types";
...
export async function POST(req: Request) {
  ...
  if (!body.imageBase64?.trim() || !body.mimeType?.trim()) {
    return Response.json({ error: "Vui lòng tải lên ảnh render cần đánh giá" }, { status: 400 });
  }
  ...
  try {
    const result = await analyzeRender(body);
